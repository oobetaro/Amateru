...
/**
 * 複数のNatalデータを選択したり編集したりできるようにするパネル。
 ...
 * @author 大澤義鷹
 */
class ChartDataPanel extends javax.swing.JPanel
    ...
    JMenu historyMenu = new JMenu("ヒストリー");
     ...
     */
    public ChartDataPanel(TimeController tcont,ChartPane iframe) {
        initComponents();
        init();
        this.iframe = iframe;
        this.tcont = tcont;
    ...
    public void setNatal( List<Natal> list ) {
        ...
        compositNatal = list.get(0).getChartType().equals(Natal.COMPOSIT) ?
                                                      list.get(0) : null;
        ...
        SKIP:
            for(int i=0; i < list.size(); i++) {
                if(list.get(i).getChartType().equals(Natal.COMPOSIT)) {
                    ...
                    //IDで検査してdataListに重複するデータがある場合は追加しない。
                    //ただしNEED_REGISTデータは除く
                    if ( id != Natal.NEED_REGIST ) {
                        for ( int j=0; j < dataList.size(); j++ ) {
                            if ( dataList.get(j).getNatal().getId() == id )
                                continue SKIP;
                        }
                    }
     ...
     * 時間を加算する。
     * 値に負数を指定すると減算も行われる。
     * @return 加減算後のカレンダーオブジェクト
     */
    public GregorianCalendar addCalendar( int day,
                                           int hour,
                                           int minute,
                                           int second ) {
        GregorianCalendar cal = timePlacePanel.addCalendar(
    ...
    public void setCalendar( GregorianCalendar cal ) {
        timePlacePanel.setCalendar( cal );
        setButtonActionPerformed( null );
    }
    ...
    public void initDataList( List<Data> dataList ) {
        ...
        if ( data.getNatal().getTime() == null ) {
            ...
            timePlacePanel.setTime( Time.valueOf( defaultTime ) );
        }
        createHistoryMenu();
        eventStop = false;
    }
    ...
    public Transit getTransit() {
        throw new java.lang.UnsupportedOperationException("廃止予定メソッド");
        //return transit;
    }
    ...
    public void setTabIcon() {
        if ( ! isComposit() ) {
            ...
            if ( data != null ) {
                iframe.setIcon(data.getNatal());
                iframe.setTitle(data.getNatal().getName());
            }
        } else {
            iframe.setIcon( getComposit() );
     ...
     * 「時間メニュー→トランジットをヒストリーに追加」から呼び出される。これは
...
//  //メニューで選択されたヒストリーをトランジットにセットする
    ...
    /**
     * このパネルのEnabled / Disenabledをセットする。Disenabledにセットすると、
     * ボタンが押せなくなり、入力フィールドにも入力できなくなる。
     */
    public void setEnabled( boolean b ) {
        timePlacePanel.setEnabled( b );
        nameComboBox.setEnabled( b );
    ...
    private void initComponents() {
        ...
        delButton.setIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/images/deleteNormal.png")));
        delButton.setToolTipText("\u9078\u629e\u4e2d\u306e\u30c7\u30fc\u30bf\u3092\u9664\u5916");
        delButton.setBorderPainted(false);
        delButton.setContentAreaFilled(false);
        delButton.setFocusPainted(false);
        delButton.setMargin(new java.awt.Insets(0, 0, 0, 0));
        delButton.setPressedIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/images/deletePressed.png")));
        delButton.setRolloverEnabled(true);
        delButton.setRolloverIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/images/deleteRollover.png")));
    ...
    // 直入力のセットボタン  //コンボボックス内のデータ削除
  private void delButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_delButtonActionPerformed
      if(compositNatal == null) {
          if(dataList.size() <= 1) {
              JOptionPane.showMessageDialog(iframe,"最後のデータは削除できません",
                  "データリストの削除",JOptionPane.ERROR_MESSAGE);
              return;
          }
      } else {
          if(dataList.size() <= 2) {
              JOptionPane.showMessageDialog(iframe,"コンポジットデータは2件以下にはできません",
                  "データリストの削除",JOptionPane.ERROR_MESSAGE);
              return;
          }
      }
      int num = nameComboBox.getSelectedIndex();
      if( JOptionPane.showConfirmDialog(iframe,
  ...
  private void nameComboBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_nameComboBoxActionPerformed
      ...
      if ( data.getNatal().getTime() == null ) {
          ...
          timePlacePanel.setTime(Time.valueOf(dt));
      }
      createHistoryMenu();
      tcont.set();
  ...
  //ネイタルフォームのセットボタン
  //現在選択されているネイタルのDataに対して、このパネルの日時をセットする。
  private void setButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_setButtonActionPerformed
      int num = nameComboBox.getSelectedIndex();
      TimePlace tp = null;
      if ( num >= 0 ) {
          tp = dataList.get(num).getTimePlace();
      } else {
          tp = new TimePlace();
      }
      timePlacePanel.getTimePlace(tp);
      tcont.set();
