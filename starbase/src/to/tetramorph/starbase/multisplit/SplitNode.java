 ...
 * 張り込むことができる。最初に作ったものは、デバイダの移動の際、他のデバイダも
 * 動いてしまう欠点があったので、それの改良品。
 * 
 * @author 大澤義鷹
 */
class SplitNode extends PlainSplitPane {
  /** 
  ...
  public void addComponent(Component comp,int insert,Component targetComp) {
    ...
    if( top == targetComp ) {
      ...
      throw new IllegalArgumentException("指定されたtargetCompが見つからない");
   ...
   * getComponentCount()は下の階層までの総数を返すが、これはこのノードのleft,right
   * のみしかカウントしない。
   */
  ...
  /**
   * 格納されているコンポーネントが一つのとき、そのコンポーネントを返す。
   */
  public Component getWhichComponent() {
    if(leftComponent == null && rightComponent == null) return null;
    if(leftComponent != null && rightComponent != null) return null;
    if(leftComponent != null) return leftComponent;
    if(rightComponent != null) return rightComponent;
    return null;
  }
}
