...
public class NodeApogeeTable {
    ...
    private GBoxLayout nodeApogeeLayout;
    private Color listOtherColor;

    /**
     * NodeApogeeTable オブジェクトを作成する
     */
    public NodeApogeeTable(ZodiacPanel zodiacPanel) {
    ...
    public void updateNodeApogee( int [] bodyIDs,
        ...
        zodiacPanel.removeGLayout(nodeApogeeLayout);
        int [] bodys = bodyIDs; //scp.getBodyIDs( scp.getBodyListMode() );
        nodeApogeeLayout = new GBoxLayout(GBoxLayout.HORIZONTAL);
        for(int id : bodys) {
            if(id == NODE || id == SOUTH_NODE) {
                nodeLabel = new GSymbolLabel("\u00CB", " " + nodeTypeName );
                nodeLabel.setColor(listOtherColor);
                nodeApogeeLayout.add( nodeLabel );
                break;
            }
        }
        for(int id: bodys) {
            if(id == APOGEE || id == ANTI_APOGEE) {
                ...
                nodeApogeeLayout.add(apogeeLabel);
                break;
            }
        }
        if( nodeApogeeLayout.getComponentCount() > 0 ) {
            nodeApogeeLayout.setLocation( 100, 70, G_RIGHT, G_BOTTOM);
            zodiacPanel.addGLayout( nodeApogeeLayout );
        }
    }
