 ...
 * @author 大澤義鷹
 */
public class TransitNameTable extends GBoxLayout {
    ...
    private GLabel tDateLabel = new GLabel( G_RIGHT );
    ...
    private void createTransitLayout() {
        ...
        add( tNameLabel, tDateLabel, tTimeZoneLabel, tPlaceLabel, tLocationLabel );
        setLocation( 100, 0, G_RIGHT, G_TOP );
//        layout = new GBoxLayout( GBoxLayout.VERTICAL );
...
//    }
    ...
    public void setTransit( Transit transit ) {
        ...
        String lat = AngleConverter
                     .getFormattedLatitude( transit.getLatitude() );
        String lon = AngleConverter
                     .getFormattedLongitude( transit.getLongitude() );
        String loc = ( transit.getLatitude() == null ) ?
                 "〔観測点不明〕"     :     lat + ", " + lon;
        tLocationLabel.setText( loc );
        int dst_offset = transit.getCalendar().get(Calendar.DST_OFFSET);
        String displayName = transit.getTimeZone().getDisplayName();
        if ( dst_offset > 0 ) {
            displayName = displayName.concat("〔夏時間実施中〕");
        }
        tTimeZoneLabel.setText( displayName );
        String date = transit.getFormattedDate();
        if ( transit.getTime() == null ) date += "〔時刻不明〕";
